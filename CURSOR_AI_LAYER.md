...
.cursor/
  ...
  skills/
    ...
    prisma-monorepo/SKILL.md
    nest-new-endpoint/SKILL.md
    next-server-client-data/SKILL.md
    signal-orchestrator/
      SKILL.md
      COORDINATION.md
      EXAMPLE.md
...
**Правило уровня репозитория (всегда включено):** `@.cursor/rules/token-economy.mdc` — режим Auto, выбор быстрых моделей для рутины, явная рекомендация Opus для тяжёлых задач, точечный контекст и отсутствие копипаста правил в чат.
...
- Для реализации атомарных задач опирайся на **доменные skills** (`scenario-creator`, `observability-scenario`, `rhf-tanstack-form`, `prisma-monorepo`, `nest-new-endpoint`, `next-server-client-data`) — не дублируй их чеклисты вручную в промпте.
...
| Схема/миграции Prisma в корне, generate/migrate из `apps/backend` | skill **`prisma-monorepo`** |
| Новый REST endpoint Nest (не сценарий), DTO + Swagger + логи/Sentry | skill **`nest-new-endpoint`** |
| RSC vs client, `server-api` vs axios, prefetch и TanStack Query | skill **`next-server-client-data`** + **`02-frontend-conventions.mdc`** |
...
Hooks (`.cursor/hooks.json`): **`afterFileEdit`** и **`preCompact`** подсказывают стек и экономию контекста; **`beforeShellExecution`** — напоминания при `git commit`, деструктивном Prisma и `docker compose down -v/--volumes` (без блокировки команд).
...
- `token-economy` — явно применить `@.cursor/rules/token-economy.mdc` в начале сессии или перед большим рефакторингом.
...
- `prisma-monorepo` — схема в `prisma/`, CLI из `apps/backend`.
- `nest-new-endpoint` — новый HTTP handler с observability guardrails.
- `next-server-client-data` — `server-api` vs клиентский axios и Query.
